// See https://aka.ms/new-console-template for more information

class Date
{
    int day;
    int month;
    int year;

    public Date()
    {
        day = 1;
        month = 1;
        year = 1;
    }
    public Date(int d, int m, int y)
    {
        day = d;
        month = m;
        year = y;
        IfCorrectDate();
    }

    public void IfCorrectDate()
    {
        if (year < 1)
        {
            throw new Exception("Incorrect value of YEAR.");
        }
        if (month < 1 || month > 12)
        {
            throw new Exception("Incorrect value of MONTH.");
        }
        if (month == 1 || month == 3 || month == 5 || month == 7 || month == 8 || month == 10 || month == 12)
        {
            if (day > 31 || day < 1)
            {
                throw new Exception("Incorrect value of DAY.(1)");
            }
        }
        if (month == 4 || month == 6 || month == 9 || month == 11)
        {
            if (day > 30 || day < 1)
            {
                throw new Exception("Incorrect value of DAY.(2)");
            }
        }
        if (month == 2)
        {
            if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)
            {
                if (day > 29 || day < 1)
                {
                    throw new Exception("Incorrect value of DAY.(3)");
                }
            }
            else
            {
                if (day > 28 || day < 1)
                {
                    throw new Exception("Incorrect value of DAY.(4)");
                }
            }
        }
    }


    public int Day
    {
        get
        {
            return day;
        }
        set
        {
            day = value;
            IfCorrectDate();
        }
    }
    public int Month
    {
        get
        {
            return month;
        }
        set
        {
            month = value;
            IfCorrectDate();
        }
    }
    public int Year
    {
        get
        {
            return year;
        }
        set
        {
            year = value;
            IfCorrectDate();
        }
    }
    public int Day_Of_Week()
    {
        if (month == 1 || month == 2)
        {
            month += 12;
            year--;
        }
        int iWeek = (day + 2 * month + 3 * (month + 1) / 5 + year + year / 4 - year / 100 + year / 400) % 7;
        switch (iWeek)
        {
            case 0:
                Console.WriteLine("Monday");
                break;
            case 1:
                Console.WriteLine("Tuesday");
                break;
            case 2:
                Console.WriteLine("Wednesday");
                break;
            case 3:
                Console.WriteLine("Thursday");
                break;
            case 4:
                Console.WriteLine("Friday");
                break;
            case 5:
                Console.WriteLine("Saturday");
                break;
            case 6:
                Console.WriteLine("Sunday");
                break;
        }
        return iWeek;
    }
    public int NumberOfDays(Date dt)
    {
        int x = 0, y = 0;
        int a = day;
        int b = month;
        int c = year;
        int d = dt.day;
        int e = dt.month;
        int f = dt.year;

        int leap = c;
        int k = 0;
        while (leap <= f)
        {

            if (leap % 4 == 0 && leap % 100 != 0 || leap % 400 == 0)
            {
                k = k + 1;
            }
            leap++;
        }
        int m1 = 31, m2 = 28, m3 = 31, m4 = 30, m5 = 31, m6 = 30, m7 = 31, m8 = 31, m9 = 30, m10 = 31, m11 = 30, m12 = 31;

        if (b == 1) { x = a; }
        else if (b == 2) { x = m1 + a; }
        else if (b == 3) { x = m1 + m2 + a; }
        else if (b == 4) { x = m1 + m2 + m3 + a; }
        else if (b == 5) { x = m1 + m2 + m3 + m4 + a; }
        else if (b == 6) { x = m1 + m2 + m3 + m4 + m5 + a; }
        else if (b == 7) { x = m1 + m2 + m3 + m4 + m5 + m6 + a; }
        else if (b == 8) { x = m1 + m2 + m3 + m4 + m5 + m6 + m7 + a; }
        else if (b == 9) { x = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + a; }
        else if (b == 10) { x = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + a; }
        else if (b == 11) { x = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + m10 + a; }
        else if (b == 12) { x = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + m10 + m11 + a; }

        if (e == 1) { y = d; }
        else if (e == 2) { y = m1 + d; }
        else if (e == 3) { y = m1 + m2 + d; }
        else if (e == 4) { y = m1 + m2 + m3 + d; }
        else if (e == 5) { y = m1 + m2 + m3 + m4 + d; }
        else if (e == 6) { y = m1 + m2 + m3 + m4 + m5 + d; }
        else if (e == 7) { y = m1 + m2 + m3 + m4 + m5 + m6 + d; }
        else if (e == 8) { y = m1 + m2 + m3 + m4 + m5 + m6 + m7 + d; }
        else if (e == 9) { y = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + d; }
        else if (e == 10) { y = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + d; }
        else if (e == 11) { y = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + m10 + d; }
        else if (e == 12) { y = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + m10 + m11 + d; }

        if ((c % 4 == 0 && leap % 100 != 0 || leap % 400 == 0) && b > 2) { k = k - 1; }
        if ((f % 4 == 0 && leap % 100 != 0 || leap % 400 == 0) && d <= 29 && e < 3) { k = k - 1; }

        if (c == f)
        {
            return ((y - x) + k);
        }
        else
            return ((f - c) * 365 + ((y - x) + k));
    }
    public void ChangeDate(int n)
    {
        if (n > 0)
        {
            for (int i = 0; i < n; i++)
            {
                if (month == 12 && day == 30)
                {
                    day = 1;
                    month = 1;
                    year++;
                }
                else if (day == 30)
                {
                    day = 1;
                    month++;
                }
                else day++;

            }
        }
        else if (n < 0)
        {
            for (int i = 0; i < -n; i++)
            {
                if (month == 1 && day == 1)
                {
                    day = 30;
                    month = 12;
                    year--;
                }
                else if (day == 1)
                {
                    day = 30;
                    month--;
                }
                else day--;

            }
        }
    }

    public void Print()
    {
        Console.WriteLine($"--------------------------");
        Console.WriteLine($"|  {day}  |  {month}\t|  {year}  |");
        Console.WriteLine($"--------------------------\n");
    }

    public static Date operator ++(Date number)
    {
        Date result = new Date();
        result.day = number.day + 1;
        result.month = number.month;
        result.year = number.year;

        if (result.month == 12 && result.day == 32)
        {
            result.day = 1;
            result.month = 1;
            result.year++;
        }

        if ((result.day == 32) && (result.month == 1 || result.month == 3 || result.month == 5 || result.month == 7 || result.month == 8 || result.month == 10))
        {
            result.day = 1;
            result.month++;
        }

        if ((result.day == 31) && (result.month == 4 || result.month == 6 || result.month == 9 || result.month == 11))
        {
            result.day = 1;
            result.month++;
        }

        if (result.month == 2)
        {
            if ((result.year % 4 == 0 && result.year % 100 != 0) || result.year % 400 == 0)
            {
                if (result.day == 30)
                {
                    result.day = 1;
                    result.month++;
                }
            }
            else
            {
                if (result.day == 29)
                {
                    result.day = 1;
                    result.month++;
                }
            }
        }

        return result;
    }

    public static Date operator --(Date number)
    {
        Date result = new Date();
        result.day = number.day - 1;
        result.month = number.month;
        result.year = number.year;

        if (result.month == 1 && result.day == 0)
        {
            result.day = 31;
            result.month = 12;
            result.year--;
        }

        if ((result.day == 0) && (result.month - 1 == 1 || result.month - 1 == 3 || result.month - 1 == 5 || result.month - 1 == 7 || result.month - 1 == 8 || result.month - 1 == 10))
        {
            result.day = 31;
            result.month--;
        }

        if ((result.day == 0) && (result.month - 1 == 4 || result.month - 1 == 6 || result.month - 1 == 9 || result.month - 1 == 11))
        {
            result.day = 30;
            result.month--;
        }

        if (result.month - 1 == 2)
        {
            if ((result.year % 4 == 0 && result.year % 100 != 0) || result.year % 400 == 0)
            {
                if (result.day == 0)
                {
                    result.day = 29;
                    result.month--;
                }
            }
            else
            {
                if (result.day == 0)
                {
                    result.day = 28;
                    result.month--;
                }
            }
        }

        return result;
    }

    public static bool operator <(Date dt1, Date dt2)
    {
        if (dt1.year < dt2.year)
        {
            return true;
        }
        else if (dt1.year == dt2.year && dt1.month < dt2.month)
        {
            return true;
        }
        if (dt1.year == dt2.year && dt1.month == dt2.month && dt1.day < dt2.day)
        {
            return true;
        }
        else return false;
    }

    public static bool operator >(Date dt1, Date dt2)
    {
        if (dt1.year > dt2.year)
        {
            return true;
        }
        else if (dt1.year == dt2.year && dt1.month > dt2.month)
        {
            return true;
        }
        if (dt1.year == dt2.year && dt1.month == dt2.month && dt1.day > dt2.day)
        {
            return true;
        }
        else return false;
    }

    public static bool operator ==(Date dt1, Date dt2)
    {
        if (dt1.year == dt2.year && dt1.month == dt2.month && dt1.day == dt2.day)
            return true;
        else return false;
    }

    public static bool operator !=(Date dt1, Date dt2)
    {
        if (dt1.year != dt2.year || dt1.month != dt2.month || dt1.day != dt2.day)
            return true;
        else return false;
    }

    public static Date operator +(Date dt, int number)
    {
        Date result = new Date();
        result.day = dt.day;
        result.month = dt.month;
        result.year = dt.year;
        if (number > 0)
        {
            for (int i = 0; i < number; i++)
            {
                result++;
            }
        }
        else
        {
            for (int i = 0; i < -number; i++)
            {
                result--;
            }
        }
        return result;
    }


    public static int operator -(Date dt1, Date dt2)
    {
        Date result = new Date();


        int count = 0;
        Date tmp = dt1;

        if (dt1 > dt2)
        {
            while ((dt1 == dt2) == false)
            {
                --dt1;
                count++;
            }
        }
        else
        {
            while ((dt1 == dt2) == false)
            {
                ++dt1;
                count++;
            }
        }
        dt1 = tmp;
        return count;
    }

    static void Main()
    {
        try
        {
            bool flag = true;

            while (flag == true)
            {
                Console.Clear();

                Console.WriteLine($"Choose the number of home work:" +
                    $"\n1 - home work 4 (class Date)" +
                    $"\n2 - home work 5 (exceptions, incorrect date)" +
                    $"\n3 - home work 6 (overloaded operations) " +
                    $"\n0 - exit ");

                string str = Console.ReadLine();
                int choice = Convert.ToInt32(str);

                switch (choice)
                {
                    case 0:
                        {
                            flag = false;
                            break;
                        }
                    case 1:
                        {
                            try
                            {
                                //constructors:
                                Date d1 = new Date();
                                d1.Print();//print of date
                                Date d2 = new Date(22, 11, 2003);
                                d2.Print();
                                Console.WriteLine();

                                //properties
                                d2.Day = 12;
                                d2.Month = 3;
                                d2.Year = 1988;
                                d2.Print();
                                Console.WriteLine($"{d2.Day}\t{d2.Month}\t{d2.Year}\n");
                                int dow = d2.Day_Of_Week();
                                Console.WriteLine();

                                //number of days
                                Date d3 = new Date(22, 11, 1990);
                                int n = d2.NumberOfDays(d3);
                                Console.WriteLine(n);
                                Console.WriteLine();

                                //changing date
                                d3.ChangeDate(12);
                                d3.Print();
                                d3.ChangeDate(-40);
                                d3.Print();
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine(ex.Message);
                            }
                            Console.ReadLine();
                            break;
                        }
                    case 2:
                        {
                            try
                            {
                                Console.WriteLine("Enter the first date:");
                                Console.Write("\nDay: ");
                                int d = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Month: ");
                                int m = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Year: ");
                                int y = Convert.ToInt32(Console.ReadLine());

                                Console.WriteLine("The first date: ");
                                Date dt3 = new Date(d, m, y);
                                dt3.Print();

                                Console.WriteLine("\n\nEnter the second date: ");
                                Console.Write("\nDay: ");
                                dt3.Day = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Month: ");
                                dt3.Month = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Year: ");
                                dt3.Year = Convert.ToInt32(Console.ReadLine());

                                Console.WriteLine("The second date: ");
                                dt3.Print();

                                Console.WriteLine("Both dates are correct. ");

                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine(ex.Message);
                            }
                            Console.ReadLine();

                            break;
                        }
                    case 3:
                        {
                            try
                            {
                                Date dt5 = new Date(22, 11, 2003);
                                Date dt6 = new Date(10, 11, 2022);
                                Console.WriteLine("\nThe first date: ");
                                dt5.Print();
                                Console.WriteLine("The second date: ");
                                dt6.Print();
                                int number = dt5 - dt6;
                                Console.WriteLine("The numbers of days between date1 and date2 = {0}.", number); ;

                                Console.Write("\nEnter the number of days. (example: 3, 41 or -365):  ");
                                int days = Convert.ToInt32(Console.ReadLine());
                                Console.WriteLine("\ndate1 + {0} days = ", days);
                                dt5 = dt5 + days;
                                dt5.Print();

                                Console.Write("\nIncrements/decrements");

                                Console.WriteLine("\ndate1:");
                                dt5.Print();

                                Console.WriteLine("\noperations: date1++, ++date1, date1--, --date1");
                                dt5++;
                                dt5.Print();

                                ++dt5;
                                dt5.Print();

                                dt5--;
                                dt5.Print();

                                --dt5;
                                dt5.Print();

                                Console.WriteLine("Enter the first date:");
                                Console.Write("\nDay: ");
                                dt5.Day = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Month: ");
                                dt5.Month = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Year: ");
                                dt5.Year = Convert.ToInt32(Console.ReadLine());

                                Console.WriteLine("\n\nEnter the second date: ");
                                Console.Write("\nDay: ");
                                dt6.Day = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Month: ");
                                dt6.Month = Convert.ToInt32(Console.ReadLine());
                                Console.Write("Year: ");
                                dt6.Year = Convert.ToInt32(Console.ReadLine());

                                Console.WriteLine("\n\ndate1:");
                                dt5.Print();
                                Console.WriteLine("date2:");
                                dt6.Print();

                                if (dt5 > dt6)
                                {
                                    Console.WriteLine("dt1 > dt2");
                                }
                                else if (dt5 < dt6)
                                {
                                    Console.WriteLine("dt1 < dt2");
                                }
                                if (dt5 == dt6)
                                {
                                    Console.WriteLine("dt1 == dt2");
                                }
                                else if (dt5 != dt6)
                                {
                                    Console.WriteLine("dt1 != dt2");
                                }
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine(ex.Message);
                            }
                            Console.ReadLine();

                            break;
                        }
                }
            }

        }
        catch (Exception ex)
        {
            Console.WriteLine(ex.Message);
        }
        Console.Read();

    }
}